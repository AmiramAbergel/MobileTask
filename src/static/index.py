...
def main():
    ...
    Initialize DB
    """
    Patient_New_List_Json_File_Path = 'local_json/Patients_list.json'
    Doctor_New_List_Json_File_Path = 'local_json/Doctors_list.json'
        ...
        if user_mode == "1":  # Patient mode
                ...
                if Password == "1234":
                    ...
                    if PatientOptions == "1":  # Patient mode --> add new patient
                        try:  # To avoid non integer input
                            ...
                            print("Enter name of doctor from list: \n|Doctor Mark Boris|\n|Doctor Clara|\n|Doctor Olivia|\n ")
                            doctor_name = input("")
                            patient_phone = input("Enter patient age                       : ")
                            patient_message = input("Enter patient gender                    : ")
                            patient_arrival_time = input("Enter arrival time without am\pm (Format 00:00 hours:minutes ): ")
                            res_patient = Patient(patient_id, patient_name, doctor_name, patient_phone, patient_message,
                    ...
                    if PatientOptions == "2":  # Patient mode --> available doctors
                        ...
                        print("Make sure that you pressed 1 first! and did first signup")
                        print("Make sure that you pressed 1 first! and did first signup")
                        print("Make sure that you pressed 1 first! and did first signup")
                        # Appointment
                        appointment_id = random_num
                        print("Appointments should be between 01:00PM to 10:00PM, Please enter a time between working hours : ")
                        appointment_start_time = input("Enter appointment start time              : ")
                        appointment_end_time = appointment_start_time  # need to be fixed
                        appointment_type = input("Enter appointment type                  : ")
                        # Patient
                        appointment_patient_id = int(input("Enter patient ID : "))
                        print("Enter id of doctor from list: \n|Doctor Mark Boris(id):123321123321 |\n|Doctor Clara(id):749375123331 |\n|Doctor Olivia(id):839621123351\n: ")
                        doctor_id = input("")

                        # Parsed Doctor Object
                        appointment_doctor_info = DoctorsListFlow().get_doctor_by_id(int(doctor_id))
                        ...
                        if appointment_doctor_info.doctor_available_status:  # Waiting list condition
                            print('Hello, we want to inform you that the doctor is available')
                            ...
                            print('Hello, we want to inform you that the doctor is Un-Available')
            ...
            while True:
                if Password == "4321":
                    ...
                    print(
                        "|To view the list of waiting patients sorted by arrival time at the hospital doctor Enter 2 |")
                    print("|To be Back Enter E                     |")
                    print("-----------------------------------------")
                    ...
                    if DoctorOptions == "1":  # Doctor mode --> List of waiting patients sorted by arrival time
                        try:  # To avoid non integer input
                            ...
                            doctor_available_status = input(
                                "Enter doctor availability(True = available, False = Not available  : ")
                            doctor_available_start_time = input(
                                "Doctors working hours should be between 01:00PM to 10:00PM, Please enter a time between working hours : ")
                            doctor_end_time = input("Session ends at : ")
                            doctor_specialty = input(" Please enter Doctor Department  : ")
                            print(
                                " Please enter id of patients that waiting for you (if exist) with space between each one : ")
